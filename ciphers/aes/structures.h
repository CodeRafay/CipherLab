...
// Helper function to print a 16-byte state as a 4x4 matrix
// Note: AES state is column-major.
// state[0] state[4] state[8]  state[12]
// state[1] state[5] state[9]  state[13]
// state[2] state[6] state[10] state[14]
// state[3] state[7] state[11] state[15]
// This function prints it in that column-major order for accurate representation.
void printState(unsigned char * state) {
	std::cout << std::hex << std::setfill('0'); // Print in hex, pad with 0
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 4; j++) {
			std::cout << "0x" << std::setw(2) << (int)state[i + 4 * j] << " ";
		}
		std::cout << std::endl;
	}
	std::cout << std::dec << std::setfill(' ') << std::endl; // Reset to decimal
}

// Helper function to print a 16-byte round key
void printRoundKey(unsigned char * key) {
	std::cout << std::hex << std::setfill('0'); // Print in hex
	for (int i = 0; i < 16; i++) {
		std::cout << "0x" << std::setw(2) << (int)key[i] << " ";
	}
	std::cout << std::dec << std::setfill(' ') << std::endl; // Reset to decimal
}
